module BaseAttachmentTests
  ...
  def test_should_create_file_from_uploaded_file_1
    assert_created do
      ...
      assert_nil attachment.width  if attachment.respond_to?(:width)
      assert_nil attachment.height if attachment.respond_to?(:height)
    end
  end
  
  def test_should_create_file_from_merb_temp_file_1
    assert_created do
  ...
  def test_should_create_file_from_uploaded_file_2
    assert_created do
      ...
      assert  attachment.image?
      assert !attachment.size.zero?
      assert_equal 1787, attachment.size
      assert_not_nil attachment.width  if attachment.respond_to?(:width)
      assert_not_nil attachment.height if attachment.respond_to?(:height)
    end
  end

  def test_should_create_file_from_merb_temp_file_2
    assert_created do
      attachment = upload_merb_file :filename => '/files/rails.png'
      assert_valid attachment
      assert !attachment.db_file.new_record? if attachment.respond_to?(:db_file)
      assert  attachment.image?
      assert !attachment.size.zero?
